...
@Component({
  ...
  styleUrls: ['./chart-client.component.css']
})
 ...
 userconnect=JSON.parse(localStorage.getItem("userconnect")!)
  ...
  ngOnInit(): void {
    this.api.getalltasks().subscribe((res:any)=>{
      this.done=res.filter((el:any)=>el.status=="done"  &&  el.client1!=null && el.client1.id==this.userconnect.id )
      console.log("list of done tasks",this.done)
      this.totaldone=this.done.length;
   
    console.log("nombre de task done", this.totaldone)
    this.api.getalltasks().subscribe((res:any)=>{
      this.todo=res.filter((el:any)=>el.status=="todo" &&  el.status== "todo" && el.client1.id==this.userconnect.id  &&  el.ing!=null)
      console.log("list of todo tasks",this.todo)
      this.totaltodo=this.todo.length;
    console.log("nombre de task todo", this.totaltodo)
   
    this.api.getalltasks().subscribe((res:any)=>{
      this.inprogress=res.filter((el:any)=>el.status=="inprogress" &&  el.client1!=null && el.client1.id==this.userconnect.id)
